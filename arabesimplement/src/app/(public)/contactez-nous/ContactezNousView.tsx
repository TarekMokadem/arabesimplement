...
function defaultMessageForWaitlist(sujet: string): string {
  if (!sujet.trim()) return "";
  if (sujet.toLowerCase().includes("liste d'attente")) {
    return (
      "Bonjour,\n\n" +
      "Je souhaite être prévenu(e) lorsque cette formation sera disponible à l'inscription.\n\n" +
      "Cordialement,"
    );
  }
  return (
    "Bonjour,\n\nJe vous contacte au sujet indiqué ci-dessus.\n\nCordialement,"
  );
}

export function ContactezNousView({
  initialSujet,
}: {
  initialSujet: string;
}) {
  ...
  const {
    ...
    defaultValues: {
      ...
      telephone: "",
      sujet: initialSujet,
      message: defaultMessageForWaitlist(initialSujet),
    },
  });
  ...
  const onSubmit = async (data: ContactInput) => {
    ...
    try {
      const { submitContact } = await import(
        "@/app/(public)/actions/contact.actions"
      );
